...
export const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        setPrograms(data.slice(0, 3)); // Featured programs
      } catch (error) {
  ...
  const featuredPrograms = programs; // Use loaded programs
          ...
          {featuredPrograms.map((prog, i) => (
            <div key={i} className="bg-white rounded-[24px] sm:rounded-[32px] overflow-hidden border border-gray-100 flex flex-col group hover:shadow-2xl hover:shadow-amber-500/5 transition-all">
              <div className="relative h-48 sm:h-56 overflow-hidden">
                <img src={prog.image_url || ''} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" alt={prog.title} />
              ...
              <div className="p-6 sm:p-8 flex-1 flex flex-col space-y-4">
                <div className="flex flex-wrap gap-2">
                  {prog.tags.map(t => (
                    <span key={t} className={`text-[9px] sm:text-[10px] font-bold px-2 py-0.5 rounded ${t === 'HYBRID' || t === 'ONLINE' ? 'bg-blue-50 text-blue-600' :
                ...
                <p className="text-[11px] sm:text-[12px] text-gray-500 leading-relaxed font-medium line-clamp-2 italic opacity-80">{prog.description}</p>

                <div className="grid grid-cols-2 gap-y-3 pt-2">
                  <div className="flex items-center gap-2 text-[10px] sm:text-[11px] text-gray-500 font-bold">
                    <Calendar className="w-3.5 h-3.5 text-gray-300" /> {prog.date_text}
                  </div>
                  <div className="flex items-center gap-2 text-[10px] sm:text-[11px] text-gray-500 font-bold">
                    <Users className="w-3.5 h-3.5 text-gray-300" /> {prog.seats_text}
                  </div>
                </div>

                <div className="pt-6 mt-auto border-t border-gray-50 flex items-center justify-between">
                  <div className="space-y-0.5">
                    <span className="block text-lg sm:text-xl font-black text-gray-900">{prog.price_text}</span>
                  </div>
            ...
            {[
              ...
              <li key={item.label} className="flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  <div className={`w-2 h-2 rounded-full shrink-0 ${item.status === 'Done' ? 'bg-green-500' :
